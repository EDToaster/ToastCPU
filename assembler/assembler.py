        ...
        if isinstance(self.words[0], Opcode):
            ...
            if opcode == "call" or opcode == "call!":
                ...
                if not isinstance(label, Label):
                    raise Exception("call macro is supposed to have one argument that is a label")
                return [
                    Instruction(f". imov r0 .{label.name}  [{self.text}]", self.labels,
                                [Opcode("imov"), Register(0), LabelMask(label, 0x00FF, 0)]),
                    Instruction(f"| imoh r0 .{label.name}", [],
                                [Opcode("imoh"), Register(0), LabelMask(label, 0xFF00, 8)]),
                ...
                return [
                    Instruction(f". imov [{self.text}]", self.labels,
                                [Opcode("imov"), reg, LabelMask(label, 0x00FF, 0)]),
                ...
                return [
                    Instruction(f". imov [{self.text}]", self.labels, [Opcode("imov"), reg, LabelMask(dest, 0x00FF,
                                                                                                      0) if is_label else Number(
                        dest.number & 0x00FF)]),
                    Instruction("' imoh", [], [Opcode("imoh"), reg, LabelMask(dest, 0xFF00, 8) if is_label else Number(
                        (dest.number & 0xFF00) >> 8)]),
                ]
            elif opcode == "isr!":
                """
                push r0
                ...
                is_label = isinstance(dest, Label)
                return [
                    Instruction(f". imov [{self.text}]", self.labels, [Opcode("imov"), Register(0),
                                                                       LabelMask(dest, 0x00FF,
                                                                                 0) if is_label else Number(
                                                                           dest.number & 0x00FF)]),
                    Instruction("| imoh", [], [Opcode("imoh"), Register(0),
                                               LabelMask(dest, 0xFF00, 8) if is_label else Number(
                                                   (dest.number & 0xFF00) >> 8)]),
                    Instruction("' cmp", [], [Opcode("cmp"), reg, Register(0)]),
                ]
            else:
                return [
                    Instruction("  " + self.text, self.labels,
                                [(LabelMask(l, 0xFFFF, 0) if isinstance(l, Label) else l) for l in self.words])
                ]
        if isinstance(self.words[0], Number):
            return [
                self
...
class Program:
    ...
    def parse_register(self, token) -> Optional[Register]:
        ...
        if num == 0:
            raise Exception("r0 is reserved for the assembler")
        return Register(num)
    ...
    def parse(self, lines: List[str]) -> list[tuple[str, str]]:
        ...
        for line in lines:
            ...
            if line_to_parse.startswith("\"") and line_to_parse.endswith("\""):
                ...
                raw_format += [(str(c.encode('ascii'))[2:-1], [Number(ord(c))]) for c in
                               raw_string.encode('utf-8').decode('unicode_escape')]
                continue
...
def main():
    ...
    lines = preprocess(input_file, {input_file})
