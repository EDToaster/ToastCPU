...
fn resolve_generic_type(
    ...
    match out_t {
        Type::Pointer(i, t) => Ok(Type::Pointer(
            *i,
            Box::new(resolve_generic_type(t.borrow(), span, generics)?),
        )),
        Type::Generic(label) => Ok(generics
            .get(label)
            .ok_or((
                *span,
                format!("Output generic {label:?} has no corresponding input type."),
            ))?
            .clone()),
        Type::Function(FunctionType { in_t, out_t }) => Ok(Type::Function(FunctionType {
            in_t: in_t
                .iter()
...
pub fn check_and_apply_multiple_stack_transitions(
    s: &str,
    span: &Span,
...
pub fn check_and_apply_stack_transition(
    s: &str,
    span: &Span,
    stack_view: &mut Stack<Type>,
    in_t: &Vec<Type>,
